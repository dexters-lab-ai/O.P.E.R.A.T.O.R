...
import { PlanningAIResponse, AIElementLocatorResponse, Insight, PlanningAction, ExecutionTaskProgressOptions, Executor, InsightExtractParam, InsightAssertionResponse, PlanningActionParamWaitFor, OnTaskStartTip, GroupedActionDump, InsightAction, ExecutionDump, AgentAssertOpt, AgentWaitForOpt } from '@midscene/core';
import { d as WebUIContext, W as WebPage, e as WebElementInfo } from './page-1f2132ee.js';
...
type CacheGroup = {
    matchCache: <T extends 'plan' | 'locate' | 'ui-tars-plan'>(pageContext: WebUIContext, type: T, actionPrompt: string) => T extends 'plan' ? PlanTask['response'] : T extends 'locate' ? LocateTask['response'] : UITarsPlanTask['response'];
...
declare class TaskCache {
     ...
     */
    matchCache(pageContext: WebUIContext, type: 'plan', userPrompt: string, cacheGroup: AiTasks): PlanTask['response'];
    matchCache(pageContext: WebUIContext, type: 'ui-tars-plan', userPrompt: string, cacheGroup: AiTasks): UITarsPlanTask['response'];
    matchCache(pageContext: WebUIContext, type: 'locate', userPrompt: string, cacheGroup: AiTasks): LocateTask['response'];
...
declare class PageTaskExecutor {
    ...
    runPlans(title: string, plans: PlanningAction[], options?: ExecutionTaskProgressOptions): Promise<ExecutionResult>;
...
declare class PageAgent<PageType extends WebPage = WebPage> {
    ...
    aiTap(targetPrompt: string, searchArea?: string): Promise<any>;
    aiHover(taskPrompt: string): Promise<any>;
    aiInput(where: string, value: string): Promise<any>;
    aiKeyboardPress(where: string, value: string): Promise<any>;
    aiAction(taskPrompt: string): Promise<void>;
    aiQuery(demand: any): Promise<any>;
    aiAssert(assertion: string, msg?: string, opt?: AgentAssertOpt): Promise<_midscene_core.InsightAssertionResponse | undefined>;
