...
function adaptQwenBbox(bbox, errorMsg) {
  if (bbox.length < 2) {
...
function adaptDoubaoBbox(bbox, width, height, errorMsg) {
  assert(
    width > 0 && height > 0,
  ...
  if (bbox.length === 6 || bbox.length === 2) {
    return [
      Math.round(bbox[0] * width / 1e3),
      Math.round(bbox[1] * height / 1e3),
      Math.round(bbox[0] * width / 1e3) + defaultBboxSize,
      Math.round(bbox[1] * height / 1e3) + defaultBboxSize
    ];
  }
  if (bbox.length === 8) {
    return [
      Math.round(bbox[0] * width / 1e3),
      Math.round(bbox[1] * height / 1e3),
      Math.round(bbox[4] * width / 1e3),
      Math.round(bbox[5] * height / 1e3)
    ];
  }
  const msg = errorMsg || `invalid bbox data for doubao-vision mode: ${JSON.stringify(bbox)} `;
  throw new Error(msg);
}
...
async function describeUserPage(context, opt) {
  ...
  const pageJSONDescription = vlLocateMode() ? "" : `Some of the elements are marked with a rectangle in the screenshot, some are not. 
...
var vlCurrentLog = `"log": string, // Log what the next one action (ONLY ONE!) you can do according to the screenshot and the instruction. The typical log looks like "I will use action {{ action-type }} to do ..". If no action should be done, log the reason. ". Use the same language as the user's instruction.`;
...
var debugCall = getDebug("ai:call");
...
var debugInspect = getDebug2("ai:inspect");
...
async function AiLocateElement(options) {
  ...
  if ("bbox" in res.content && Array.isArray(res.content.bbox)) {
    const errorMsg = res.content.errors?.length ? `Failed to parse bbox: ${res.content.errors?.join(",")}` : "";
...
//# sourceMappingURL=chunk-QVNVYZ3P.js.map